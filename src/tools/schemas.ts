...
export const idempotencyField = {
  idempotencyKey: z
    ...
    .optional()
    .describe(
      "Optional idempotency key. If omitted, the MCP server generates one for mutating requests.",
    ),
};
...
export const correlationField = {
  correlationKey: z
    ...
    .optional()
    .describe(
      "Optional caller correlation key echoed in async handoffs. Defaults to the idempotency key when omitted.",
    ),
};

export const asyncControlFields = {
  ...waitFields,
  ...idempotencyField,
  ...correlationField,
};
...
const employeeCountSchema = z
  ...
  .refine(
    ...
    {
      message:
        "employeeCount.min must be less than or equal to employeeCount.max.",
    },
  );
