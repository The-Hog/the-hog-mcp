...
test("delete monitor requires confirmation", async () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "delete_monitor",
  );
  assert.ok(tool);
  await assert.rejects(
    () =>
      tool.execute({ id: "mon_1" }, {
...
test("async primitive tools strip MCP controls, set idempotency, and poll when requested", async () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "search_companies",
  );
  assert.ok(tool);
  ...
  assert.equal(requests[1]?.path, "/api/operations/op_123");
  assert.equal(typeof requests[1]?.timeoutMs, "number");
  assert.ok((requests[1]?.timeoutMs ?? 0) > 0);
  assert.ok((requests[1]?.timeoutMs ?? Number.POSITIVE_INFINITY) <= 5000);
...
test("async primitive tools do not repoll terminal initial responses", async () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "search_people",
  );
  assert.ok(tool);
...
test("async primitive tools do not repoll nested terminal initial responses", async () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "submit_search",
  );
  assert.ok(tool);
...
test("async primitive tools return a top-level continuation on forced timeout", async () => {
  ...
  assert.equal(continuation.maxRecommendedPollSeconds, 180);
  assert.match(String(idempotencyKey), /^search_people_[a-f0-9]{32}$/);
...
test("scrape_web_pages requires at least one url or item", async () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "scrape_web_pages",
  );
  assert.ok(tool);
...
test("scrape_web_page forwards requested formats and schema-guided extraction fields", async () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "scrape_web_page",
  );
  assert.ok(tool);
...
test("mutating primitive tools expose idempotency and risk annotations", async () => {
  ...
  for (const tool of mutatingTools) {
    assert.ok(
      tool.inputSchema.idempotencyKey,
      `${tool.name} exposes idempotencyKey`,
    );
    assert.deepEqual(
      {
    ...
    await tool.execute(
      ...
      },
      {
        request: async (request: {
          ...
          requests.push(request);
          return {
            data: { id: "result_1", operationId: "op_1" },
            status: 200,
            requestId: null,
          };
        },
...
test("read-only and destructive primitive tools expose risk annotations", () => {
  ...
  const deleteMonitor = primitiveTools.find(
    (tool) => tool.name === "delete_monitor",
  );
...
test("start_deep_research schema matches the public request body", () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "start_deep_research",
  );
  assert.ok(tool);
  assert.deepEqual(Object.keys(tool.inputSchema).sort(), [
    "correlationKey",
  ...
  await tool.execute(
    ...
    },
    {
      request: async (request: { body?: unknown; idempotencyKey?: string }) => {
        requests.push(request);
        return {
          data: { id: "search_1", status: "queued" },
          status: 202,
          requestId: "req_1",
        };
      },
...
test("enrichment polling uses the enrichment ID from queued responses when requested", async () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "enrich_contacts",
  );
  assert.ok(tool);
...
test("async primitive tools poll the correct status endpoint when requested", async () => {
  ...
  for (const item of cases) {
    ...
    assert.ok(
      tool.inputSchema.waitForResult,
      `${item.name} exposes waitForResult`,
    );
    assert.ok(
      tool.inputSchema.timeoutSeconds,
      `${item.name} exposes timeoutSeconds`,
    );
...
test("async primitive tools return a continuation when polling is disabled", async () => {
  ...
  const { message, idempotencyKey, correlationKey, ...handoff } =
    result as Record<string, unknown>;
  assert.match(
    String(message),
    /Poll get_operation with operationId "op_people"/,
...
test("async primitive continuation echoes explicit correlation keys and strips them from the request body", async () => {
  const tool = primitiveTools.find(
    (candidate) => candidate.name === "search_people",
  );
  assert.ok(tool);
  ...
  const result = await tool.execute(
    {
      ...
      waitForResult: false,
      idempotencyKey: "idem_people",
      correlationKey: "corr_people",
    },
    {
      request: async (request: { body?: unknown; idempotencyKey?: string }) => {
        requests.push(request);
  ...
  assert.equal(requests[0]?.idempotencyKey, "idem_people");
  assert.deepEqual(requests[0]?.body, { query: "security engineers" });
  assert.equal(
    (result as Record<string, unknown>).idempotencyKey,
    "idem_people",
  );
  assert.equal(
    (result as Record<string, unknown>).correlationKey,
    "corr_people",
  );
});
...
test("configured async primitive tools poll operation IDs even without an initial status", async () => {
  ...
  const requests: Array<{ method: string; path: string }> = [];
  await tool.execute({ query: "security engineers", waitForResult: true }, {
    request: async (request: { method: string; path: string }) => {
      requests.push(request);
      if (request.method === "POST") {
        return {
...
test("primitive tools infer operation polling from queued operationId responses", async () => {
  ...
  const requests: Array<{ method: string; path: string }> = [];
  const result = await tool.execute({ waitForResult: true }, {
    request: async (request: { method: string; path: string }) => {
      requests.push(request);
      if (request.method === "POST") {
        return {
...
test("primitive tools infer operation polling from nested queued operation responses", async () => {
  ...
  const requests: Array<{ method: string; path: string }> = [];
  await tool.execute({ waitForResult: true }, {
    request: async (request: { method: string; path: string }) => {
      requests.push(request);
      if (request.method === "POST") {
        return {
...
test("linkedin primitive tools send public OpenAPI-shaped request bodies", async () => {
  ...
  for (const item of cases) {
    ...
    const requests: Array<{ body?: unknown; idempotencyKey?: string }> = [];
    await tool.execute({ ...item.input, idempotencyKey: `idem_${item.name}` }, {
      request: async (request: { body?: unknown; idempotencyKey?: string }) => {
        requests.push(request);
