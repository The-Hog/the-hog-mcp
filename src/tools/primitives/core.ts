...
export const corePrimitiveTools: PrimitiveToolDefinition[] = [
  ...
  endpointTool({
    name: "enrich_contact",
    description:
      "Enrich one person with requested contact fields or signals. Use this when the user has a LinkedIn URL, email, X handle, or GitHub username and needs enrichment. This may consume The Hog credits. Async enrichments return a continuation by default; set waitForResult=true to poll inline within the safe timeout budget.",
    method: "POST",
  ...
  endpointTool({
    ...
    description:
      "Batch enrich people with requested contact fields or signals. Use this when the user has multiple LinkedIn URLs, emails, X handles, or GitHub usernames. This may consume The Hog credits. Async enrichments return a continuation by default; set waitForResult=true to poll inline within the safe timeout budget.",
    method: "POST",
    path: "/api/enrichments",
    endpointPath: "/api/enrichments",
    inputSchema: {
  ...
  endpointTool({
    ...
    description:
      "Start a structured deep research job with a prompt and JSON Schema. Use this when the user needs sourced research returned in a specific JSON shape. This may consume The Hog credits and returns a continuation by default; set waitForResult=true to poll inline within the safe timeout budget.",
    method: "POST",
