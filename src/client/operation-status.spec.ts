...
test("readStatus normalizes top-level and nested statuses", () => {
  ...
  assert.equal(
    readStatus({ data: { status: "PARTIAL_SUCCESS" } }),
    "partial_success",
  );
...
test("operation status classification keeps async and terminal sets disjoint", () => {
  ...
  assert.equal(isTerminalStatus("not_found"), true);
  assert.equal(isTerminalStatus("unauthorized"), true);
  assert.equal(isTerminalStatus("malformed"), true);
});
